...
function_which_throws = mock.Mock(side_effect=throw_exception)


class RetryTests(TestCase):
    def tearDown(self):
        super().tearDown()
        function_which_throws.reset_mock()

    def test_reraises(self):
        with self.assertRaises(TestException):
            functions.retry(function_which_throws)
        # the default retry_count is 3, this can be changed
        self.assertEqual(function_which_throws.call_count, 3)

    def test_respects_retry_count(self):
        with self.assertRaises(TestException):
            functions.retry(function_which_throws, retry_count=4)
        self.assertEqual(function_which_throws.call_count, 4)

    def test_calls_post_exception(self):
        post_exception = mock.Mock()
        with self.assertRaises(TestException):
            functions.retry(function_which_throws, post_exception=post_exception)
        # the default retry_count is 3, this can be changed
...
class ARetryTests(IsolatedAsyncioTestCase):
    ...
    async def test_calls_post_exception(self):
